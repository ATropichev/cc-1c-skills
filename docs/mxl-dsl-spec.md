...
Объект-элемент читается как ячейка, если несёт хоть один её ключ (`span`, `rowspan`, `style`,
`param`, `detail`, `text`, `template`), и как многоязычный текст в противном случае.
