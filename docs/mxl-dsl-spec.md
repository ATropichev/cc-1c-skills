...
| `control` | нет | `input` | Элемент управления поля ввода: `input` или `checkbox`. Только вместе с `valueType` |
...
`control` нужен редко: умолчание платформы — поле ввода, и оно применяется ко всем типам, включая
`Boolean`. Флажок задаётся явно. Значение `"none"` (тега элемента управления нет вовсе) — форма
раундтрипа, для ручного авторинга не нужна.
...
- начальное значение ячейки-поля ввода и сериализованные настройки её элемента управления
  (тип значения при этом переживает цикл);
