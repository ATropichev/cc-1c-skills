  ...
  "databases": [
    {
      ...
      "repository": {
        "path": "\\\\srv01\\repo\\MyApp",     // каталог хранилища или адрес сервера хранилища
        "user": "Ivanov",
        "password": ""
      },
...
|------|-----|:-----:|----------|
| `repository.path` | string | да | Каталог хранилища или адрес сервера хранилища |
...
> **Сетевое хранилище.** Путь передаётся платформе как есть, поэтому подходит любая форма,
> которую она принимает, — в том числе адрес сервера хранилища. Конкретный синтаксис в
> документации, которой мы располагаем (7.4.15 «Команды работы с хранилищем конфигурации»,
> «Приложение 4»), не описан, и на серверном хранилище навык не проверялся: под рукой нет
> сервера хранилища. Файловое хранилище проверено полностью.
