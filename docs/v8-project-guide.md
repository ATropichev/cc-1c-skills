...
> **Сетевое хранилище.** Адрес — `tcp://<хост>[:<порт>]/<имя>`, порт по умолчанию 1542; хранилище
> обслуживает сервер хранилища (`crserver`). В документации, которой мы располагаем (7.4.15
> «Команды работы с хранилищем конфигурации», «Приложение 4»), эта форма не описана — проверена
> на стенде 8.3.23: цикл, файл списка объектов и администрирование работают так же, как на
> файловом. Недоступный сервер даёт «Соединение с хранилищем конфигурации не установлено» — то же
> сообщение, что и при отсутствии реквизитов.
