...
«Замерено» = создана пустая ИБ этой платформой и выгружена ею же; результат прочитан из `ConfigDumpInfo.xml` и корня `Configuration.xml`. Строки «нет» — интерполяция по шагу лестницы: платформы 8.3.21 и 8.3.22 в наличии не было. Версия `2.16` подтверждена сторонним сообщением (issue #63), а не собственным замером.
...
Дельта вниз от `2.17` измерена только в одну точку: пустая конфигурация `2.13` (8.3.20) не содержит `AllowedIncomingShareRequestTypes`, `DatabaseTablespacesUseMode` и `DefaultReportAppearanceTemplate`. На какой именно ступени 2.14–2.17 они появились — не установлено, поэтому в реестр «тег → минимальная версия» (`meta-validate`, проверка 18) они не внесены.
