...
1. Прочитай `.v8-project.json` из корня проекта и применяй `.v8-project.local.json`, если он существует. Возьми итоговый `v8path` и разреши базу:
2. Если пользователь указал параметры подключения (путь, сервер) — используй напрямую
3. Если указал базу по имени — ищи по id / alias / name в итоговой конфигурации проекта
