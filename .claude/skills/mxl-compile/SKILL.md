...
{ columns, page, defaultWidth, columnWidths, columnStyles,
  ...
  areas: [{ name, columnSet, rows: [{ height, hidden, rowStyle, cells: [
    { col, span, rowspan, style, param, detail, text, template, valueType, controlType }
  ]}]}],
...
- `valueType` делает ячейку полем ввода (`"Number(15,3,nonneg)"`, `"String(10)"`, `"CatalogRef.Валюты"`, составной через ` + `); текста в такой ячейке быть не может, а `controlType: "checkbox"` меняет поле ввода на флажок
