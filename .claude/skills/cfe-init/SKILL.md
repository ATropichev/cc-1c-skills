...
1. Прочитай `.v8-project.json` из корня проекта и применяй `.v8-project.local.json`, если он существует
...
4. Если у базы нет `configSrc`, но есть корневой `configSrc` — используй его
5. Если `configSrc` нет — спроси у пользователя
