...
**Переподключение** базы, которая уже была подключена, обычно требует обоих флагов: конфигурация
в базе не пустая (`-ForceReplaceCfg`), а за пользователем хранилища всё ещё числится эта база
(`-ForceBindAlreadyBindedUser`). Навык подсказывает оба по сообщению платформы.
